...
Agent는 boto3를 통해 CloudWatch에 직접 메트릭을 전송하며, Grafana Business Overview 대시보드에서 시각화됩니다.

```
AgentCore Runtime → boto3 → CloudWatch (CDCI/AgentCore) → Grafana
```
...
| 메트릭 | 설명 |
|---|---|
| `agent_invocation_total` | 에이전트 호출 횟수 (`status`: success/error) |
| `agent_latency_seconds` | `/invocations` 엔드포인트 응답 시간 |
| `agent_token_input_total` | LLM 입력 토큰 수 (`model_id` 포함) |
| `agent_token_output_total` | LLM 출력 토큰 수 (`model_id` 포함) |
| `tool_execution_total` | 각 Step 실행 횟수 (`tool_name`: kb_retrieval/step1_llm/nutrient_calc/step3_llm) |
| `tool_execution_duration_seconds` | 각 Step 실행 시간 |

CloudWatch Namespace: `CDCI/AgentCore` / `agent_name`: `analysis-agent`

> IAM 권한(`cloudwatch:PutMetricData`)은 인프라 레포(`codecaine-terraform-infra`)의 `agentcore/analysis-agent/iam.tf`에서 관리.
> 배포 후 약 2~3분 뒤 CloudWatch에서 확인 가능.
