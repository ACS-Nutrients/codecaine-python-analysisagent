...
Agent는 OpenTelemetry를 통해 메트릭을 수집하며, ADOT 사이드카 → CloudWatch → Grafana 경로로 시각화됩니다.
...
| `agent_token_input_total` | LLM 입력 토큰 수 |
| `agent_token_output_total` | LLM 출력 토큰 수 |
| `tool_execution_total` | 각 Tool 실행 횟수 (`tool_name`: kb_retrieval/step1_llm/nutrient_calc/step3_llm) |
| `tool_execution_duration_seconds` | 각 Tool 실행 시간 |

CloudWatch Namespace: `CDCI/AgentCore` / `agent_name`: `analysis-agent`

> ADOT 사이드카와 IAM 권한은 인프라 레포(`codecaine-terraform-infra`)에서 관리.
